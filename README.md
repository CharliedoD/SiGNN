...
1. **双向泄漏积分发放 (BLIF) 神经元**：扩展了传统 LIF 神经元，支持双向放电机制和自适应阈值更新
2. **时序感知 (TA) 层**：结合邻域聚合与脉冲神经元，在每一跳聚合中引入时序动态
3. **多粒度时序聚合**：通过多个具有不同时间分辨率的通道同时处理时序快照，捕获不同尺度的时间模式
4. **混合邻域采样**：同时从累积图和演化图中采样邻居节点，平衡历史信息与即时变化
...
模型使用 **Micro-F1** 在验证集上进行早停选择，并报告对应的测试集 **Macro-F1** 和 **Micro-F1**。
